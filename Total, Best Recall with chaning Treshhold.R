...
library(readr)
data <- read_csv("D:/MSc - Georgia Tech/Spring 2024/Data Analytics Business - MGT-6203-OAN/Project/My Part/Total, 2014.csv")
...
model <- glm(final_result ~ gender + region + highest_education + imd_band + age_band + num_of_prev_attempts + studied_credits + disability, family = binomial(link = "logit"), data = trainData)
...
predictions_class <- ifelse(predictions > 0.65, 1, 0)
